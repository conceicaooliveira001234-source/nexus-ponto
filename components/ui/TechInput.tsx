...
const TechInput: React.FC<TechInputProps> = ({ label, icon, rightIcon, onRightIconClick, className = '', ...props }) => {
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      <div className="relative group">
        ...
        <input
          {...props}
          className={`w-full bg-slate-950/50 border border-slate-700 text-white placeholder-slate-600 text-sm rounded-lg focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 block pl-10 ${rightIcon ? 'pr-10' : 'pr-3'} p-3 transition-all duration-300 backdrop-blur-sm outline-none`}
        />
        {rightIcon && (
          <div 
